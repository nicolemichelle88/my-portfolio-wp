...
class STWSettingsForm extends SettingsForm 
{
	...
	public function __construct ($paramList, $settingPrefix, $formID = false)
	{
		...
		foreach ($paramList as $fieldName => $fieldDetails)
		{
			...
			if (isset($fieldDetails['default']) && empty($settings[$fieldName])) {  	
	...
	protected function processPost()
	{
		...
		if ($accountLevel == 'invalid') {
			$this->formObj->setElementHTML('stwwt_account_level', 
				sprintf('<span class="stwwt_account_invalid">'.__('<span class="stwwt_account_level">Invalid</span> - please provide valid account details.', 'shrinktheweb-website-preview-plugin').'</span>'));
		} 
		// Valid account
		else 
		{ 
	...
	private function checkAccountType($access_id, $secret_key)
	{
		...
		$fetchURL = urldecode("$protocol//images.shrinktheweb.com/account.php?".http_build_query($args));
		$resp = wp_remote_get($fetchURL);	
		
		// Check there are no errors
		if (is_wp_error($resp) || 200 != $resp['response']['code'] ) 
		{	
			echo $this->showMessage(__('Error fetching account details.', 'shrinktheweb-website-preview-plugin') . $resp->get_error_message(), true);
			return false;
		}
